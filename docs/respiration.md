...
| `--plot` | 关 | 保存三联图 PNG |
| `--f-min` / `--f-max` | 1.0 / ≈4.17 Hz | 呼吸频段，默认 **60–250 次/分** |
...
默认在 **60–250 次/分**（**1.0–4.17 Hz**）内搜索主峰并做带通；与「低频约 60、高频不超过 250」一致。可按动物状态用 `--f-min` / `--f-max` 调整。
...
5. 收窄或放宽频段：例如 `--f-min 1 --f-max 4.17`（对应 60–250 次/分）  
